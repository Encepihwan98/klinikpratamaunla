    ...
    <v-main>
      <div class="app-content-container pa-6">
        <v-card class="mx-auto" max-width="97%" elevation="5">
          <v-card-text>
            <p class="text-h6 text--primary">Pasien Operasi Management</p>
          </v-card-text>
          <v-container>
            <v-row>
            ...
            <template v-slot:default>
              ...
              <tbody v-if="data.data.data.length > 0 && web.isTableLoad == false">
                <tr v-for="(item, index) in data.data.data" :key="item.name">
                  ...
                  <td>{{item.waktu}}</td>
                  <td>
                    <v-btn icon :to="{ name: 'operasi-detail', params: { id: item.id }}">
...
export default {
  ...
  methods: {
    ...
    filterPage(sort_by) {
      ...
      let url =
        ...
        "/api/v1/layanan-penunjang-operasi/" +
        "?page=" +
        this.filter.page +
        "&limit=" +
        this.filter.limit +
  ...
  created() {
    ...
    this._url = window.location.origin + "/api/v1/layanan-penunjang-operasi/";
