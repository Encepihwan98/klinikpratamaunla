    ...
    <v-main>
      <div class="app-content-container boxed-container pa-6">
        <v-card class="mx-auto" max-width="97%" elevation="5">
          <v-card-text>
            <p class="text-h6 text--primary">Daftar Pegawai</p>
          </v-card-text>
          <v-container>
            <v-row>
            ...
            <template v-slot:default>
              <thead>
                <tr>
                  ...
                  <th class="text-left">Nomor Telp</th>
              ...
              >
                <v-container>
                  <v-row>
                    ...
                    <v-col class="d-flex" cols="12" sm="6">
                      <v-select
                        :items="qualification.items"
                        v-model="pegawai.kualifikasi"
                        @input="changeID('qualification')"
                        label="Kualifikasi:"
...
export default {
  ...
  methods: {
    changeID(event) {
      if (event == "position") {
        let currentID = this.pegawai.jabatan;
        this.position.data.forEach((v) => {
          if (v.description == currentID) {
            this.pegawai.position_id = v.id;
          }
        });
      }else if (event == "qualification") {
        let currentID = this.pegawai.kualifikasi;
        this.qualification.data.forEach((v) => {
          if (v.description == currentID) {
            this.pegawai.qualification_id = v.id;
          }
        });
      }
    },
    ...
    setSelectedJabatan() {
      axios.get(`/api/v1/list-jabatan/`).then((res) => {
        console.log(res);
        if (res.status === 200) {
          res.data.data.forEach((v) => {
            this.position.items.push(v.description);
          });
          this.position.data = res.data.data;
        } else {
    ...
    setSelectedKualifikasi() {
      axios.get(`/api/v1/list-kualifikasi/`).then((res) => {
        console.log(res);
        if (res.status === 200) {
          res.data.data.forEach((v) => {
            this.qualification.items.push(v.description);
          });
          this.qualification.data = res.data.data;
        } else {
          this.makeDefaultNotification(
            response.data.status,
            response.data.message
    ...
    store() {
      let req = Object.assign(this.pegawai, this.filter);
      this.currentData = null;
      axios
        .post(this._url, req)
        .then((response) => {
    ...
    update() {
      let req = Object.assign(this.pegawai, this.filter);
      axios
        .put(`${this._url}${this.pegawai.id}`, req)
        .then((response) => {
          if (response.status == 200) {
            this.dialog.state = false;
      ...
      axios
        ...
        .then((response) => {
          if (response.status == 200) {
            ...
            this.position.data.forEach((v) => {
              if (v.id == response.data.data.position_id) {
                this.pegawai.jabatan = v.description;
                this.pegawai.position_id = v.id;
              }
            }); 
            this.pegawai = response.data.data;
            this.qualification.data.forEach((v) => {
              if (v.id == response.data.data.qualification_id) {
                this.pegawai.kualifikasi = v.description;
                this.pegawai.qualification_id = v.id;
              }
            }); 
          }
  ...
  created() {
    ...
    this._url = window.location.origin + "/api/v1/daftar-pegawai/";
    this.filterPage("");
  },
  computed: {
    dialogState() {
  ...
  watch: {
    dialogState: function (n, o) {
      console.log(n);
      this.setSelectedJabatan();
      this.setSelectedKualifikasi();
