    ...
    <v-main>
      <div class="app-content-container boxed-container pa-6">
        <v-card class="mx-auto" max-width="97%" elevation="5">
          <v-card-text>
            <p class="text-h6 text--primary">Daftar Kualifikasi</p>
          </v-card-text>
          <v-container>
            <v-row>
                ...
                <v-container>
                  <v-row>
                    <v-col class="d-flex" cols="12" sm="12">
                      <v-text-field
                        v-model="kualifikasi.description"
                        label="Uraian :"
                        placeholder="Uraian"
                        outlined
                        dense
                    ...
                    <v-col class="d-flex" cols="12" sm="12">
                      <v-select
                        :items="categori.items"
                        label="Kualifikasi"
                        outlined
                        dense
                        v-model="kualifikasi.kategori"
                        @input="changeID('categori')"
                        :error-messages="errors.name"
                        :rules="[rules.required]"
                        :disabled="condition == 'show' ? true : false"
                      ></v-select>
...
export default {
  ...
  methods: {
    changeID(event) {
      if (event == "categori") {
        let currentID = this.kualifikasi.kategori;
        this.categori.data.forEach((v) => {
          if (v.description == currentID) {
            this.kualifikasi.category_id = v.id;
          }
        });
      }
    },
    ...
    selectCategori() {
      axios.get(`/api/v1/list-kategori-kualifikasi/`).then((res) => {
        console.log(res);
        if (res.status == 200) {
          res.data.data.forEach((v) => {
            this.categori.items.push(v.description);
          });
          this.categori.data = res.data.data;
        }else {
    ...
    store() {
      let req = Object.assign(this.kualifikasi, this.filter);
      this.currentData = null;
      axios
        .post(this._url, req)
        .then((response) => {
    ...
    update() {
      let req = Object.assign(this.kualifikasi, this.filter);
      axios
        .put(`${this._url}${this.kualifikasi.id}`, req)
        .then((response) => {
          if (response.status == 200) {
            this.dialog.state = false;
    ...
    show(id) {
      ...
      axios
        ...
        .then((response) => {
          if (response.status == 200) {
            this.kualifikasi = response.data.data;
            this.categori.data.forEach((v) => {
              if (v.id == response.data.data.category_id) {
                this.kualifikasi.kategori = v.description;
                this.kualifikasi.category_id = v.id;
              }
            }); 
          }
  ...
  created() {
    ...
    this._url = window.location.origin + "/api/v1/kualifikasi/";
    this.filterPage("");
  },
  computed: {
    dialogState() {
