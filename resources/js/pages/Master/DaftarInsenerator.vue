    ...
    <v-main>
      <div class="app-content-container boxed-container pa-6">
        <v-card class="mx-auto" max-width="97%" elevation="5">
          <v-card-text>
            <p class="text-h6 text--primary">Daftar Insenerator</p>
          </v-card-text>
          <v-container>
            <v-row>
            ...
            <template v-slot:default>
              <thead>
                <tr>
                  ...
                  <th class="text-left">
                    Unit
                    <v-icon @click="filterPage('unit')" x-small>{{
                      filter.sortBy == "unit"
                        ? filter.orderBy == "asc"
                          ? "fas fa-sort-amount-up"
                          : "fas fa-sort-amount-down"
              ...
              >
                <v-container>
                  <v-row>
                    ...
                    <v-col class="d-flex" cols="12" sm="12">
                       <v-text-field
                        v-model="incenerator.description"
                        label="Uraian :"
                        placeholder="Uraian"
                        outlined
                        dense
...
export default {
  ...
  methods: {
    ...
    store() {
      let req = Object.assign(this.incenerator, this.filter);
      this.currentData = null;
      axios
        .post(this._url, req)
        .then((response) => {
    ...
    update() {
      let req = Object.assign(this.incenerator, this.filter);
      axios
        .put(`${this._url}${this.incenerator.id}`, req)
        .then((response) => {
          if (response.status == 200) {
            this.dialog.state = false;
    ...
    show(id) {
      ...
      axios
        ...
        .then((response) => {
          if (response.status == 200) {
            this.incenerator = response.data.data;
          }
        })
        .catch((e) => {
          this.errorState(e);
  ...
  created() {
    ...
    this._url = window.location.origin + "/api/v1/daftar-insenerator/";
    this.filterPage("");
  },
  computed: {
    dialogState() {
