        ...
        <v-main>
                ...
                <v-container>
                    <v-row>
                        <v-col cols="12" sm="12">
                            <v-card>
                                <v-card-text>
                                    <p class="text-h6"> Daftar Apoteker</p>
                                </v-card-text>
                                <v-row>
            ...
            <div>
                <v-dialog v-model="dialog.state" persistent max-width="600px">
                    <v-card>
                        <v-card-title class="text-h5"> Pegawai Apoteker </v-card-title>

                        <v-form class="mx-3 my-3" ref="form" v-model="valid" lazy-validation :currentData="currentData">
                            <v-container>
                                <v-row>
                                    <v-col class="pa-0 mr-6" cols="12" sm="12">
                                        <v-select :items="users.items" @input="changeID('users')" v-model="apoteker.nama"
                                            label="User Id" dense outlined></v-select>
                                    </v-col>
                                    <v-col class="pa-0 mr-6" cols="12" sm="12">
                                        <v-select :items="['laki-laki', 'perempuan']" v-model="apoteker.jenis_kelamin"
                                            label="Jenis Kelamin" dense outlined></v-select>
                                    </v-col>
                                    <v-col class="pa-0 mr-6" cols="12" sm="12">
                                        <v-text-field label="No Hp" outlined dense v-model="apoteker.no_hp" small>
                                        </v-text-field>
                                    </v-col>
                                    <v-col class="pa-0 mr-6" cols="12" sm="12">
                                        <v-text-field label="email" outlined dense v-model="apoteker.email" small>
                                        </v-text-field>
                                    </v-col>
                                    <v-col class="pa-0 mr-6" cols="12" sm="12">
                                        <v-text-field label="Alamat" outlined dense v-model="apoteker.alamat" small>
                                        </v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="12">
...
export default {
    ...
    methods: {
        changeID(event) {
            if (event == "users") {
                let currentID = this.apoteker.nama;
                this.users.data.forEach((v) => {
                    // console.log(v);
                    if (v.name == currentID) {
                        this.apoteker.user_id = v.id;
                    }
                });
            }
        },
        ...
        store() {
            let req = Object.assign(this.apoteker, this.filter);
            this.currentData = null;
            axios
                .post(this._url, req)
                .then((response) => {
        ...
        update() {
            let req = Object.assign(this.apoteker, this.filter);
            axios
                .put(`${this._url}${this.apoteker.id}`, req)
                .then((response) => {
                    if (response.status == 200) {
        ...
        show(id) {
            ...
            axios
                ...
                .then((response) => {
                    if (response.status == 200) {
                        this.apoteker = response.data.data;
                    }
                })
                .catch((e) => {
                    this.errorState(e);
    ...
    created() {
        ...
        this._url = window.location.origin + "/api/v1/apoteker/";
        this.filterPage("");
