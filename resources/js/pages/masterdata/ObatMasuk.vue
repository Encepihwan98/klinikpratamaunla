        ...
        <v-main>
                ...
                <v-container>
                    <v-row>
                        <v-col cols="12" sm="12">
                            <v-card>
                                <v-card-text>
                                    <p class="text-h6"> Daftar Obat Masuk</p>
                                </v-card-text>
                                <v-row>
            ...
            <div>
                <v-dialog v-model="dialog.state" persistent max-width="600px">
                    <v-card>
                        <v-card-title class="text-h5"> Suplier </v-card-title>
                            ...
                            <v-container>
                                <v-row>
                                    <v-col class="d-flex pa-0 mr-2" cols="12" sm="12">
                                        <v-select :items="obat.items" @input="changeID('obat')" v-model="obatMasuk.obat"
                                            label="Obat" dense outlined>
                                        </v-select>
                                    </v-col>
                                    <v-col class="d-flex pa-0 mr-2" cols="12" sm="12">
                                        <v-select :items="suplier.items" @input="changeID('suplier')"
                                            v-model="obatMasuk.suplier" label="Suplier" dense outlined>
                                        </v-select>
                                    </v-col>
                                    <v-col class="pa-0 mr-6" cols="12" sm="12">
                                        <v-text-field label="Jumlah" outlined dense v-model="obatMasuk.jumlah_masuk"
                                            small>
                                        </v-text-field>
                                    </v-col>
                                    <v-col cols="12" class="pa-0 mr-2" sm="6" md="12">
                                        <v-menu v-model="menu2" :close-on-content-click="false" :nudge-right="40"
                                            transition="scale-transition" offset-y min-width="auto">
                                            <template v-slot:activator="{ on, attrs }">
                                                <v-text-field v-model="obatMasuk.tgl" label="Tanggal"
                                                    prepend-icon="mdi-calendar" readonly v-bind="attrs" v-on="on"
                                                    outlined dense></v-text-field>
                                            </template>
                                            <v-date-picker v-model="obatMasuk.tgl" @input="menu2 = false">
                                            </v-date-picker>
                                        </v-menu>
                                    </v-col>
                                    <v-col cols="12" sm="12">
...
export default {
    ...
    methods: {
        changeID(event) {
            if (event == "obat") {
                let currentID = this.obatMasuk.obat;
                this.obat.data.forEach((v) => {
                    if (v.nama == currentID) {
                        this.obatMasuk.obat_id = v.id;
                    }
                });
            } else if (event == "suplier") {
                let currentID = this.obatMasuk.suplier;
                this.suplier.data.forEach((v) => {
                    if (v.nama == currentID) {
                        this.obatMasuk.suplier_id = v.id;
                    }
                });
            }
        },
        setSelectedSuplier() {
            axios.get(`/api/v1/list-suplier/`).then((res) => {
                if (res.status === 200) {
                    res.data.data.forEach((v) => {
                        this.suplier.items.push(v.nama);
                    });
                    this.suplier.data = res.data.data;
                } else {
    ...
    created() {
        ...
        this._url = window.location.origin + "/api/v1/daftar-obat-masuk/";
        this.filterPage("");
        this.setSelectedSuplier();
