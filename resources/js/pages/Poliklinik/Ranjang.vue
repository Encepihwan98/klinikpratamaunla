    ...
    <v-main>
      <div class="app-content-container boxed-container pa-6">
        <v-card class="mx-auto" max-width="97%" elevation="5">
          <v-card-text>
            <p class="text-h6 text--primary">Daftar Ranjang</p>
          </v-card-text>
          <v-container>
            <v-row>
            ...
            <template v-slot:default>
              <thead>
                <tr>
                  ...
                  <th class="text-left">
                    Uraian
                    <v-icon @click="filterPage('room_id')" x-small>{{
                      filter.sortBy == "room_id"
                        ? filter.orderBy == "asc"
                          ? "fas fa-sort-amount-up"
                          : "fas fa-sort-amount-down"
                        : "fas fa-sort-alt"
                  ...
                  <th class="text-left">
                    Total Ranjang
                    <v-icon @click="filterPage('bed_total')" x-small>{{
                      filter.sortBy == "bed_total"
                        ? filter.orderBy == "asc"
                          ? "fas fa-sort-amount-up"
                          : "fas fa-sort-amount-down"
                        : "fas fa-sort-alt"
              ...
              >
                <v-container>
                  <v-row>
                    ...
                    <v-col class="d-flex" cols="12" sm="12">
                      <v-text-field
                        v-model="room.bed_total"
                        label="Total Ranjang :"
                        placeholder="Total Ranjang"
                        outlined
                        dense
                        clearable
                        :error-messages="errors.name"
...
export default {
  ...
  methods: {
    changeID(event) {
      if (event == "clinicRoom") {
        let currentID = this.room.kamar;
        this.clinicRoom.data.forEach((v) => {
          if (v.name == currentID) {
            this.room.room_id = v.id;
          }
        });
      }
    },
    ...
    setSelectedRoom() {
      axios.get(`/api/v1/list-room/`).then((res) => {
        console.log(res);
        if (res.status === 200) {
  ...
  created() {
    ...
    this._url = window.location.origin + "/api/v1/ranjang/";
    this.filterPage("");
  },
  computed: {
    dialogState() {
