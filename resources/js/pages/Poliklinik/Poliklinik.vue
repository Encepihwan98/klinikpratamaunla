    ...
    <v-main>
      <div class="app-content-container boxed-container pa-6">
        <v-card class="mx-auto" max-width="97%" elevation="5">
          <v-card-text>
            <p class="text-h6 text--primary">Daftar Poliklinik</p>
          </v-card-text>
          <v-container>
            <v-row>
...
export default {
  ...
  methods: {
    changeID(event) {
      if (event == "type") {
        let currentID = this.room.jenis;
        this.type.data.forEach((v) => {
          if (v.description == currentID) {
            this.room.type_id = v.id;
          }
        });
      }
    },
    ...
    setSelectedType() {
      axios.get(`/api/v1/list-poliklinik-type/`).then((res) => {
        console.log(res);
        if (res.status === 200) {
      ...
      axios
        ...
        .then((response) => {
          if (response.status == 200) {
            ...
            this.type.data.forEach((v) => {
              if (v.id == response.data.data.type_id) {
                this.room.jenis = v.name;
                this.room.type_id = v.id;
              }
            });
          }
  ...
  created() {
    ...
    this._url = window.location.origin + "/api/v1/poliklinik/";
    this.filterPage("");
  },
  computed: {
    dialogState() {
