    ...
    <v-main>
      <div class="app-content-container boxed-container pa-6">
        <v-card class="mx-auto" max-width="97%" elevation="5">
          <v-card-text>
            <p class="text-h6 text--primary">Daftar Ruangan</p>
          </v-card-text>
          <v-container>
            <v-row>
...
export default {
  ...
  methods: {
    ...
    changeID(event) {
      if (event == "clinic") {
        let currentID = this.room.poliklinik;
        this.clinic.data.forEach((v) => {
          if (v.name == currentID) {
            this.room.polyclinic_id = v.id;
          }
        });
      }else if (event == "grade") {
        let currentID = this.room.kelas;
        this.grade.data.forEach((v) => {
          if (v.description == currentID) {
            this.room.grade_id = v.id;
          }
        });
      }
    },
  ...
  created() {
    ...
    this._url = window.location.origin + "/api/v1/ruangan/";
    this.filterPage("");
  },
  computed: {
    dialogState() {
