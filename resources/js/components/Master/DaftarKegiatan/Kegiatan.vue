    ...
    <div class="app-content-container boxed-container pa-6">
      <v-container>
        <v-row>
          ...
          <v-col class="d-flex" cols="12" sm="2">
            <v-btn depressed color="primary" @click="selectMethod(null, 'add')">
              Tambah Data
            </v-btn>
          </v-col>
        </v-row>
                ...
                <v-row>
                  ...
                  <v-col class="d-flex" cols="12" sm="12">
                    <v-select
                      ...
                      label="Kategori"
                      dense
                      v-model="agama.kategori"
                      @input="changeID('category')"
                      outlined
                    ></v-select>
                  </v-col>
                </v-row>
...
export default {
  ...
  methods: {
    changeID(event) {
      if (event == "category") {
        let currentID = this.agama.kategori;
        this.category.data.forEach((v) => {
          if (v.description == currentID) {
            this.agama.type = v.id;
          }
        });
      }
    },
    ...
    store() {
      let url = window.location.origin + "/api/v1/daftar-kegiatan/";
      ...
      axios
        ...
        .then((response) => {
          if (response.status == 200) {
            ...
            this.category.data.forEach((v) => {
              if (v.id == response.data.data.type) {
                this.agama.kategori = v.description;
                this.agama.type = v.id;
              }
            }); 
          }
    ...
    setSelectedCategory() {
      axios.get(`/api/v1/list-kategori-kegiatan/`).then((res) => {
        console.log(res);
        if (res.status === 200) {
          res.data.data.forEach((v) => {
            this.category.items.push(v.description);
          });
