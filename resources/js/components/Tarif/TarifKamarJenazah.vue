          ...
          <v-card-text>
            ...
            >
              <v-container>
                ...
                <div
                  v-for="(value, key, index) in ExaminationAction[currentData]"
                  :key="index"
                >
...
export default {
  ...
  methods: {
    ...
    store() {
      ...
      let req = Object.assign(this.ExaminationAction, this.filter, {
        module: "tarif",
      });
      this.currentData = null;
      axios
    ...
    update() {
      ...
      axios
        ...
        .put(`${this._url}${this.ExaminationAction.id}`, req)
        .then((response) => {
          if (response.status == 200) {
            this.dialog.state = false;
    ...
    show(id) {
      ...
      axios
        ...
        .then((response) => {
          if (response.status == 200) {
            this.ExaminationAction = response.data.data;
  ...
  created() {
    if (this.baseData && this.baseData.data.length > 0) {
      ...
      this.baseData.data.forEach((val) => {
        this.ExaminationAction.push(val.service_rate);
      });
    }
