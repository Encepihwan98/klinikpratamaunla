          ...
          <v-card-text>
            ...
            >
              <v-container>
                ...
                <div
                  v-for="(value, key, index) in UTDRS[currentData]"
                  :key="index"
                >
...
export default {
  ...
  methods: {
    ...
    store() {
      ...
      let req = Object.assign(this.UTDRS, this.filter, {
        module: "tarif",
      });
      this.currentData = null;
      axios
    ...
    update() {
      ...
      axios
        ...
        .put(`${this._url}${this.UTDRS.id}`, req)
        .then((response) => {
          if (response.status == 200) {
            this.dialog.state = false;
  ...
  created() {
    if (this.baseData && this.baseData.data.length > 0) {
      ...
      this.baseData.data.forEach((val) => {
        this.UTDRS.push(val.service_rate);
      });
    }
