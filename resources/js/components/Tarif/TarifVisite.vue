          ...
          <v-card-text>
            ...
            >
              <v-container>
                ...
                <div
                  v-for="(value, key, index) in VisitType[currentData]"
                  :key="index"
                >
...
export default {
  ...
  methods: {
    ...
    store() {
      ...
      let req = Object.assign(this.VisitType, this.filter, {
        module: "tarif",
      });
      this.currentData = null;
      axios
    ...
    update() {
      ...
      axios
        ...
        .put(`${this._url}${this.VisitType.id}`, req)
        .then((response) => {
          if (response.status == 200) {
            this.dialog.state = false;
    ...
    show(id) {
      ...
      axios
        ...
        .then((response) => {
          if (response.status == 200) {
            this.VisitType = response.data.data;
  ...
  created() {
    if (this.baseData && this.baseData.data.length > 0) {
      ...
      this.baseData.data.forEach((val) => {
        this.VisitType.push(val.service_rate);
      });
    }
