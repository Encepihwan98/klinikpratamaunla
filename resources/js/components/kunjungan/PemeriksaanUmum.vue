      ...
      <template v-slot:default>
        <thead>
          <tr>
            ...
            <th class="text-left">Pemeriksaan Umum</th>
        ...
        <tbody
          ...
          <tr v-for="(item, index) in useData" :key="item.index">
            <td>{{ index + (!data.from ? baseData.from : data.from) }}</td>
            <td>{{ item.treatment_time }}</td>
            <td>{{ item.description }}</td>
            <td>{{ item.examination_result }}</td>
            <td>{{ item.name }}</td>
            <td>
              <v-btn 
                small
                ...
                <v-row>
                  <v-col class="d-flex" cols="12" sm="12" md="12">
                    <v-select
                      ...
                      dense
                      outlined
                      v-model="inspection.CheckingType"
                      @input="changeID('CheckingType')"
                    ></v-select>
                  </v-col>
                  ...
                  <v-col class="d-flex" cols="12" sm="12" md="12">
                    <v-text-field
                      ...
                      required
                      v-model="inspection.examination_result"
                    ></v-text-field>
                  </v-col>
                  ...
                  <v-col cols="12" lg="12">
                    ...
                    >
                      <template v-slot:activator="{ on, attrs }">
                        <v-text-field
                          outlined
                          v-model="inspection.treatment_time"
...
export default {
  ...
  methods: {
    changeID(event) {
      if (event == "CheckingType") {
        let currentID = this.inspection.CheckingType;
        this.type.data.forEach((v) => {
          if (v.description == currentID) {
            this.inspection.general_examination_id = v.id;
          }
        });
      } else if (event == "officer") {
        let currentID = this.inspection.officer;
        this.officer.data.forEach((v) => {
          // console.log(v);
          if (v.name == currentID) {
            this.inspection.officer_id = v.id;
          }
        });
      }
    },
    ...
    setSelectPemerriksaan() {
      axios.get(`/api/v1/list-pemeriksaan-umum`).then((res) => {
        if (res.status === 200) {
          res.data.data.forEach((v) => {
            this.type.items.push(v.description);
          });
    ...
    store() {
      let req = Object.assign(this.inspection, this.filter, this.params);
      this.currentData = null;
      axios
        .post(window.location.origin + "/api/v1/layanan-pemeriksaan/", req)
        .then((response) => {
    ...
    update() {
      ...
      axios
        .put(
          `${window.location.origin + "/api/v1/layanan-pemeriksaan/"}${
            this.support.id
          }`,
          req
        )
    ...
    remove() {
      ...
      axios
        .delete(`${ window.location.origin + "/api/v1/layanan-pemeriksaan/"}${this.currentData.id}`, { data: this.filter })
        .then((response) => {
          if (response.status == 200) {
