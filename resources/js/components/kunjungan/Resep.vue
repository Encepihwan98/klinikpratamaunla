...
export default {
  ...
  methods: {
    changeID(event) {
      if (event == "Obat") {
        let currentID = this.recipe.obat;
        this.medicine.data.forEach((v) => {
          if (v.description == currentID) {
            this.recipe.medicine_id = v.id;
          }
        });
      } else if (event == "officer") {
        let currentID = this.recipe.officer;
        this.officer.data.forEach((v) => {
          if (v.name == currentID) {
            this.recipe.office_id = v.id;
          }
        });
      }
    },
    ...
    setSelectMedecine() {
      axios.get(`/api/v1/list-logistik`).then((res) => {
        if (res.status === 200) {
          res.data.data.forEach((v) => {
            this.medicine.items.push(v.description);
          });
          this.medicine.data = res.data.data;
        } else {
          this.makeDefaultNotification(
            response.data.status,
            response.data.message
    ...
    store() {
      let req = Object.assign(this.recipe, this.filter);
      this.currentData = null;
      axios
        .post(window.location.origin + "/api/v1/layanan-resep/", req)
        .then((response) => {
    ...
    update() {
      ...
      axios
        .put(
          `${window.location.origin + "/api/v1/layanan-resep/"}${
            this.support.id
          }`,
          req
        )
    ...
    show(id) {
      let url = `${window.location.origin + "/api/v1/layanan-resep/"}${id}`;
      axios
        .get(url)
        .then((response) => {
          if (response.status == 200) {
    ...
    remove() {
      ...
      axios
        .delete(`${window.location.origin + "/api/v1/layanan-resep/"}${this.currentData.id}`, { data: this.filter })
        .then((response) => {
          if (response.status == 200) {
  ...
  created() {
    ...
    this._url = window.location.origin + "/api/v1/layanan-resep/";
    this.filterPage("");
  },
  computed: {
    dialogState() {
